...
| --- | --- | --- | --- | --- |
| **Reblaze / Link11** | `server: rhino-core-shield`; cookies `waap_id`, `rbzid`; status **247/248** and **474/481/492**; body `window.rbzns` / `winsocks()` / `bereshit` | Camoufox solves the `ac_v2` JS gate **cookie-free** (Firefox skips the `isChrome()` cluster) and `waap_id`/`rbzid` get minted — but `ac_v2` is a one-shot hashcash with **no public solver**, so a warmed identity is the durable route. 474/481 = IP tier → rotate proxy | **3** | High |
| **Cloudflare** | `cf-ray`; `server: cloudflare`; cookies `__cf_bm`, `cf_clearance`; "Just a moment…" interstitial; Turnstile script `challenges.cloudflare.com` | Managed/JS challenge → clean engine mints `cf_clearance`. An **interactive Turnstile** widget → solver/identity (Tier 3) | **2** (Turnstile→3) | High |
